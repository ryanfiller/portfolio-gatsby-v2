...
``[`gatsby-plugin-react-helmet`](https://www.gatsbyjs.org/packages/gatsby-plugin-react-helmet/) is a default Gatsby plugin that comes with every new Gatsby starter site. In the starter site there are good examples of it being used to pass metadata from any page up and into the `<head>` of the site, but it actually comes with a lot of other features. The layout wrapping method I outlined above can make it kind of difficult to pass data upwards, but if you dig into the [documentation of the base package](https://github.com/nfl/react-helmet#reference-guide) there's actually a way to add attributes to the `<body>` tag of the page. I've used this to add high level classnames to the top of the page and then use my [favorite scss feature](https://css-tricks.com/the-sass-ampersand/#article-header-id-5) to to add page specific overrides.
...
This hook lets components have page specifc styles, like a homepage that needs a special grid layout, or a custom 404 page that needs to hide a header and a footer.
