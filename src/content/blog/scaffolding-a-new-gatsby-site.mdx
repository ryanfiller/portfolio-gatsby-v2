...
I've set up quite a few new Gatsby sites over the last year, and there's been a few escape hatches I've liked to leave myself every time. These definitely aren't ideas I came up with wholely on my own, but I thought an aggregate of them might be helpful for anyone setting up a Gatsby site for a first time, or even anyone struggling with an existing implementation.
...
I like being able to do page transitions, and Gatsby generously provides the [gatsby-plugin-layout](https://www.gatsbyjs.org/packages/gatsby-plugin-layout/) to bring back this v1 functionality. Since this component automatically wraps every page, I've found that its also a good place to put other global things, like styles or some context providers. I have a feeling this is using Gatsby's [`wrapPageElement` Browser API](https://www.gatsbyjs.org/docs/browser-apis/#wrapPageElement) behind the scenes, but in my opinion its a little bit easier to configure via plugin.
...
It's pretty easy to break the editor by trying to query something that isn't there, so be careful with that regex and make sure it accounts for _not_ matching anything, but its even possible to pass a custom React component into the `toPreview` value and customize how the image looks in the preview pane. This pairs super well with the MDX component I outlined above. Just [dump out the props in the `toBlock` value](https://github.com/ryanfiller/portfolio-gatsby-v2/blob/master/src/cms/editor/image.js#L63) and have the [MDX component grab them on the front end](https://github.com/ryanfiller/portfolio-gatsby-v2/blob/master/src/components/markdown/image.js#L8) and it's possible to make highly configurable React widgets that a user can easily edit.
