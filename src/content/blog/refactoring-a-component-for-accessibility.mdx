...
I use [Gatsby](https://www.gatsbyjs.org/) to build this blog, and while it uses [React](https://reactjs.org/) under the hood to generate pages, one of my favorite features about it is that it generates static html to ship to the browser. That means that even though what I have is build with React, a javascript framework, what eventually ships to a user can still be plain html. This means the `<Change>` component will still work for a user even if they have javascript disabled in their browser.
