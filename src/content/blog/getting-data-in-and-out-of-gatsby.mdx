...
serialize: ({query: {site, allMdx }}) => {
  ...
  return allMdx.edges.map(edge => {
    ...
    return {
      ...
      edge.node.frontmatter.meta.date,
      url,
      custom_elements: [
...
```
 ...
 \
These might seem obvious, but there's a couple of things I wanted to make sure and call out in the `serialize` function.

constructing a slug, dealing with image links
